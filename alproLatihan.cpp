...
struct Nilai {
	...
	Nilai(){ // Konstruktor untuk menginisialisasi gradeMap
...
class LinkedList {
	...
	public:
		...
		void deleteNode(string jurusan) {
			...
			while (temp != NULL) { // Selama masih ada node
				if (temp->mhs.jurusan == jurusan) { // Jika jurusan sama
					if (temp == head) { // Jika node yang akan dihapus adalah head
		...
		void updateNode(string nim) {
			...
			while (temp != NULL) { // Selama masih ada node
				if (temp->mhs.nim == nim) { // Jika NIM sama
					found = true; // Menandakan bahwa data ditemukan
					Nilai nilai; // Membuat objek nilai untuk grade
					
					if (temp->mhs.ipk < 3.00) { // Memeriksa apakah IPK kurang dari 3.00
		...
		void readIpk(string jurusan) {
			...
			if (!found) { // Jika tidak ada mahasiswa dengan jurusan tersebut
		...
		void sortIpk() {
			...
			do {
				...
				while (temp->next != NULL) { // Selama masih ada node berikutnya
					if (temp->mhs.ipk < temp->next->mhs.ipk) { // Jika IPK saat ini lebih kecil dari IPK berikutnya
		...
		void inputNilai(Mhs &mhs, Nilai &nilai) {
			string gradeInput;for (int i = 1; i <= 4; i++) { // Loop untuk memasukkan nilai 4 mata kuliah
				while (true) { // Loop hingga nilai valid dimasukkan
					...
					if (grade > 0 || gradeInput == "E") { // Memeriksa apakah grade valid
						if (i == 1) mhs.mk1 = grade; // Menyimpan nilai mk1
						else if (i == 2) mhs.mk2 = grade; // Menyimpan nilai mk2
						else if (i == 3) mhs.mk3 = grade; // Menyimpan nilai mk3
						else mhs.mk4 = grade; // Menyimpan nilai mk4
		...
		void getIpkHighestAndLowest() {
			...
			while (mhs != NULL) { // Selama masih ada node
				if (mhs->mhs.ipk > highest) { // Jika IPK lebih tinggi dari nilai tertinggi
					highest = mhs->mhs.ipk; // Memperbarui nilai tertinggi
					highestStudent = mhs; // Memperbarui pointer mahasiswa dengan IPK tertinggi
				}
				
				if (mhs->mhs.ipk < lowest) { // Jika IPK lebih rendah dari nilai terendah
		...
		void searchData() {
			...
			while (mhs != NULL) { // Selama masih ada node
				if (mhs->mhs.nim == nim) { // Jika NIM sama
		...
		void addMhs() {
			...
			Nilai nilai; // Membuat objek nilai untuk grade
			cout << "Nama: "; getline(cin, mhs.nama); // Menerima input nama
			cout << "NIM: "; getline(cin, mhs.nim); // Menerima input NIM
			cout << "Jurusan: "; getline(cin, mhs.jurusan); // Menerima input jurusan
...
void menu() {
	...
	do {
		...
		if (pilihan == 1) {
			...
			cout << "Masukan Jurusan: "; cin >> jurusan; // Menerima input jurusan
